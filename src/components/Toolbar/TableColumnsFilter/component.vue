            ...
            <transition-group>
              <div
                ...
                <el-checkbox flex-box="1" v-model="currentValue[index].show">
                  {{ option.label || option.prop || _text.unnamed }}
                </el-checkbox>
...
<script>
  ...
  import d2pButton from '../Button/component'
  import D2TableColumnsFixedController from '../TableColumnsFixedController/component'
    ...
    methods: {
      ...
      fixedChange (index, value) {
        ...
        if (value === 'left'&& value!== 0) {
          this.currentValue.splice(index-1,0,this.currentValue.splice(index, 1)[0])
        }
        if (value === 'right'&& value!== this.currentValue.length-1) {
          this.currentValue.splice(index+1,0,this.currentValue.splice(index, 1)[0])
        }
      },
      // 全选和反选发生变化时触发
      ...
      refresh () {
        ...
        compareSource.forEach(option => {
          ...
          if (this.storedOptions != null) {
            storeItem = option
            option = options.find(column => column.prop === option.prop)
          }
          let item = value.find(column => column.prop === option.prop)
